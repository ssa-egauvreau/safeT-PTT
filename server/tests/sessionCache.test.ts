...
/**
 * Run `fn` with `Date.now` stubbed to a controllable counter. The callback
 * receives an `advance(ms)` to push time forward without involving the
 * `node:test` mock-timer API — needed for tests written before that API
 * was adopted in this file.
 */
