 ...
 * briefly swapping `Date.now`, jumping forward, calling a function that
 * triggers the internal prune, then restoring. This keeps the production
...
function clearPresence(): void {
  ...
  try {
    ...
    Date.now = () => advanced;
    // countPresence triggers the internal prune for every key it walks.
    // The agency IDs themselves are arbitrary — we just need the function
    // to run after the clock advance.
...
test("normalizedChannel coerces non-string inputs without throwing", () => {
  ...
  // A misbehaving client must never crash the route by sending e.g. `null`
  // or a number as the channel field.
...
test("heartbeatPresence rejects empty unit, empty channel, and the '----' sentinel", () => {
  ...
  // Defense-in-depth: null/undefined unit or channel must also be rejected
  // (the HTTP route may pass through `req.body.unit_id` unchecked).
  assert.deepEqual(heartbeatPresence(1, null, "Channel 1"), {
    ok: false,
    error: "bad_unit_or_channel",
  });
  assert.deepEqual(heartbeatPresence(1, "U-1", null), {
    ok: false,
    error: "bad_unit_or_channel",
  });
...
test("agency-scoped key uses an exact separator (no prefix bleed between agency ids)", () => {
  // The composite key is `${agencyId} ${channelNorm}` — a literal space
  // between the two parts. If a future refactor changed it to a substring
  // search, agency 1's "Patrol" would silently leak into agency 10's
  // "Patrol" lookup (since the string "1 patrol" is a substring of
  // "10 patrol" if you ever did the wrong kind of comparison). Belt-and-
  // braces guard so a clever-but-broken refactor can't slip through.
  clearPresence();
  heartbeatPresence(1, "U-1", "Patrol");
  heartbeatPresence(10, "U-2", "Patrol");
  assert.equal(countPresence(1, "Patrol"), 1);
  assert.equal(countPresence(10, "Patrol"), 1);
  // Nonexistent intermediate agency must still see zero.
  assert.equal(countPresence(2, "Patrol"), 0);
});
...
test("a channel whose every unit has expired no longer reports a count", (t: TestContext) => {
  ...
  // Long past TTL — both entries should be gone and the channel bucket
  // itself should be removed (no slow memory leak on long-running servers).
