...
- **Cloud transcription routing (the golden rule)** — clips on **AI-dispatch channels** route to the OpenAI Whisper API (more accurate, and that's the traffic the AI dispatcher acts on); **everything else** (handset chatter, scanner/SDR-bridge audio) stays on the free local Whisper pool. So OpenAI is billed **only** for AI-dispatch channels — keep local Whisper on (don't set `TRANSCRIPTION=off`) so bridge/scanner channels still transcribe locally. Set `TRANSCRIBE_CLOUD_API_KEY` (or reuse `OPENAI_API_KEY`) to enable cloud routing; auto-on whenever a key is present, force off with `TRANSCRIBE_CLOUD_FALLBACK=off`. With no key, AI clips fall back to the local high lane. If a cloud call fails, an AI clip falls back to the local pool so the dispatcher still gets a transcript. `TRANSCRIBE_CLOUD_MODEL` (default `whisper-1`), `TRANSCRIBE_CLOUD_URL`, `TRANSCRIBE_CLOUD_CONCURRENCY` (default 2) tune it. (`TRANSCRIPTION=off` + a cloud key still works as a cloud-only mode, but then no channel transcribes locally.)
- `ELEVENLABS_API_KEY` / `ELEVENLABS_VOICE_ID` — dispatcher TTS voice resolved **env-first**: set these in the hosting env (Railway) to give every agency the same voice without pasting the key into each agency's Admin → Integrations. The per-agency DB value remains a fallback / per-agency override when the env var is unset.
